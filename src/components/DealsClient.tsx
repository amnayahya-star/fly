...
export default function DealsClient({ dict, deals, lang }: { dict: any, deals: any[], lang: string }) {
  const categories = [
    { id: 'all', label: dict.deals.catAll, icon: Tag },
    { id: 'flights', label: dict.deals.catFlights, icon: Plane },
    { id: 'hotels', label: dict.deals.catHotels, icon: Building2 },
    { id: 'packages', label: dict.deals.catPackages, icon: Ticket },
  ];

  const [activeCategory, setActiveCategory] = useState('all');

  const filteredDeals = deals.filter(
    (deal: any) => activeCategory === 'all' || deal.type === activeCategory
  );
  ...
  return (
    <div className="relative z-10 max-w-[1400px] mx-auto px-8 md:px-12 xl:px-24 pt-40">
      ...
      >
        ...
        <h1 className="text-4xl md:text-6xl font-bold text-white mb-6">{dict.deals.title}</h1>
        <p className="text-white/70 text-lg max-w-2xl mx-auto">{dict.deals.subtitle}</p>
      </motion.div>
